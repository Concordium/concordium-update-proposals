...
(``metadata_hash``), an ``OptionalExtRef`` (``external_reference``), a 4-byte little-endian
...
- The contract MUST reject with ``AgentRevoked`` if the agent's status is ``Revoked``.
- The contract MUST reject with ``Unauthorized`` if the caller is not the CIS-2 owner.
- The contract MUST reject with ``InvalidAgentWalletProof`` if ``deadline`` is in the past or
  outside an implementation-defined accepted validity window.
...
The transfer amount MUST be exactly one and the ``to`` field MUST be an account address.
The contract MUST reject with ``InvalidTransfer`` if the amount is not one or ``to`` is a contract
address.
...
.. list-table::
  ...
  * - ``InvalidAgentWalletProof``
    - -7212
    - The ``setAgentWallet`` signature is malformed or does not verify against ``new_wallet``'s
      account credentials, or its ``deadline`` is outside the accepted validity window.
  * - ``InvalidTransfer``
    - -7213
    - A ``transfer`` amount is not one or its ``to`` field is a contract address.
